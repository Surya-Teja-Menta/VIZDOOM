...
class ViZDoomGym(Env):
    def __init__(self,render=False,config="../../scenarios/take_cover.cfg"):
        super().__init__()
        #init Doomgame
        self.game = DoomGame()
...
def train():
    CHECKPOINT_DIR = './models/train_take_cover'
    ...
    env=ViZDoomGym(config='../../scenarios/take_cover_1.cfg')
    model = PPO('CnnPolicy', env, tensorboard_log=LOG_DIR, verbose=1, learning_rate=0.00006, n_steps=8192)
    model.learn(total_timesteps=100000,callback=callback)
    env=ViZDoomGym(config='../../scenarios/take_cover_2.cfg')
    model.set_env(env)
    model.learn(total_timesteps=100000,callback=callback)
    env=ViZDoomGym(config='../../scenarios/take_cover_3.cfg')
    model.set_env(env)
    model.learn(total_timesteps=100000,callback=callback)
    env=ViZDoomGym(config='../../scenarios/take_cover_4.cfg')
    model.set_env(env)
    model.learn(total_timesteps=100000,callback=callback)
    env=ViZDoomGym(config='../../scenarios/take_cover_5.cfg')
    model.set_env(env)
    model.learn(total_timesteps=200000,callback=callback)
    model.save('./models/take_cover_model')

def test():
    env=ViZDoomGym(render=True)
    model=PPO.load('./models/take_cover_model')
    obs=env.reset()
